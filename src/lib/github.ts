...
// email. The repo is deleted immediately after.
...
// Performs an authenticated GitHub API request, throwing a readable error on a 4xx/5xx.
...
// Builds a plausible random author name from the name pools.
...
// Generates a unique throwaway repository name.
function randomRepoName(): string {
	const suffix = Math.random().toString(36).slice(2, 10);
	return `tmp-probe-${Math.floor(Date.now() / 1000)}-${suffix}`;
}

// Returns true when a string looks like a valid email address.
export function validateEmail(email: string): boolean {
	const at = email.indexOf('@');
	return at > 0 && email.slice(at + 1).includes('.');
}

// Base64-encodes a UTF-8 string using the Node serverless runtime's Buffer.
...
// Runs a single email-to-account probe under the operator's account.
