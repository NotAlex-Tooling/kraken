...
const HEADER_EMAIL = /^(?:From|Author|Committer|Signed-off-by|Co-authored-by):.*?<([^>]+)>/gim;
...
const COMMIT_MIN = 1;
const COMMIT_MAX = 10;
const REPO_DEFAULT = 6;
const COMMIT_DEFAULT = 5;
...
// Returns a randomly chosen browser User-Agent string.
...
// Escapes regex metacharacters so dynamic repo/user names cannot over-match.
function escapeRegex(value: string): string {
	return value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

// Validates an address and rejects noreply and known placeholder domains.
...
// Fetches a URL with a rotated User-Agent, retrying once on 429 and returning null on failure.
...
// Normalises raw input into a bare GitHub username, stripping URLs and decoration.
...
// Returns true when a string is a structurally valid GitHub username.
...
// doubles as the existence check, since a missing user 404s here.
...
// Runs the full pipeline: validate, list repos, scan patches at the requested depth, then rank addresses.
